...
// Rota 1: Verifica a Key, envia o TEMPO e entrega o Script + Vigia Injetado
app.get('/verificar', async (req, res) => {
    try {
        ...
        if (content.keys[key]) {
            ...
            // Verifica expiração
            if (info.expires !== -1 && agora > info.expires) {
            ...
            // INJEÇÃO DE SEGURANÇA COM ANTI-CACHE (O segredo do kick rápido)
            const codigoSeguranca = `
                task.spawn(function()
                    local player = game.Players.LocalPlayer
                    local checkUrl = "${MY_URL}/checar?key=${key}&nocache="
                    while task.wait(5) do
                        local s, r = pcall(function() 
                            return game:HttpGet(checkUrl .. tostring(tick())) 
                        end)
            ...
            // Envia o tempo restante e o script separados por |||
